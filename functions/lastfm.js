...
export async function onRequest(context) {
  ...
  try {
    ...
    // Get max plays for color scaling and first item color
    const maxPlays = Math.max(...items.map(item => parseInt(item.playcount)));
    
    // Calculate proportional sizes based on custom width
    const fontSize = Math.max(12, Math.round(customWidth / 35));
    const titleSize = Math.max(16, Math.round(customWidth / 25));
    const rowHeight = Math.max(20, Math.round(customWidth / 25));
    const startY = Math.round(customWidth / 12);
    const headerHeight = Math.round(customWidth / 12);
    const logoSize = Math.round(titleSize * 1.2);
    ...
    const svg = `
      <svg xmlns="http://www.w3.org/2000/svg" width="${customWidth}" height="${height}" viewBox="0 0 ${customWidth} ${height}">
        ...
        <!-- Header Group -->
        <g transform="translate(25, ${headerHeight/1.8})">
          <!-- Last.fm Logo -->
          <path transform="translate(0, -${titleSize/1.5}) scale(${logoSize/800})" 
                fill="#D6D5C9" 
          ...
          <!-- Title Text -->
          <text x="${logoSize * 1.2}" class="title">Top ${showAlbums ? 'Albums' : 'Artists'} Last Week</text>
        </g>
