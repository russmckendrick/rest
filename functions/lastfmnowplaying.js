    ...
    try {
      ...
      const svg = `
        <svg xmlns="http://www.w3.org/2000/svg" width="${customWidth}" height="${height}" viewBox="0 0 ${customWidth} ${height}">
          ...
          <style>
            .title { font: bold ${titleSize}px system-ui, sans-serif; fill: #D6D5C9; filter: url(#shadow); }
            .info { font: ${fontSize}px system-ui, sans-serif; fill: #D6D5C9; filter: url(#shadow); }
            .secondary { font: ${fontSize}px system-ui, sans-serif; fill: #B9BAA3; filter: url(#shadow); }
          ...
          <!-- Content (Right Side) -->
          <g transform="translate(${artSize + contentPadding}, ${contentPadding})">
            <!-- Last.fm Logo -->
            <path transform="scale(${logoSize/25})" 
                  fill="#D6D5C9" 
            ...
            <!-- Status -->
            <text x="${logoSize * 1.5}" y="${titleSize/2}" class="title">
              ${isNowPlaying ? 'Now Playing' : 'Last Played'}
            </text>
            
            <!-- Track Information -->
            <g transform="translate(0, ${titleSize * 2})">
              ${isNowPlaying ? `
              <text class="now-playing">♫ PLAYING NOW</text>` : ''}
              <text class="info" y="${fontSize * 2}">${trackName}</text>
              <text class="secondary" y="${fontSize * 3.5}">by ${artistName}</text>
              <text class="secondary" y="${fontSize * 5}">from ${albumName}</text>
            </g>
          </g>
        </svg>
      `;
